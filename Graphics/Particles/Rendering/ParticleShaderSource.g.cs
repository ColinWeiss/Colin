// <auto-generated>由 ParticleRender.fx 生成, 请勿手改; 修改 .fx 后重新生成或用 mgfxc 重编.</auto-generated>
namespace Particle.Rendering
{
  /// <summary>粒子渲染着色器的 HLSL 源码 (嵌入常量, 供运行时编译回退使用).</summary>
  internal static class ParticleShaderSource
  {
    /// <summary>ParticleRender.fx 的完整源码.</summary>
...
// 渲染模式: 0 = 普通公告牌 (按 Rotation 旋转), 1 = 速度拉伸 (沿速度方向).
float StretchedMode;
...
struct ParticleInstance
{
    ...
    float4 Extra : TEXCOORD3;           // 基准尺寸, 保留...
};
...
VertexShaderOutput MainVS(VertexShaderInput input)
{
    ...
    if (StretchedMode > 0.5)
    {
        // —— 速度拉伸: 沿速度方向展开, 长度 = 尺寸 * 长宽比 + |速度| * 系数 (封顶) ——
        float speed = length(velocity);
        float2 direction = speed > 0.0001 ? velocity / speed : float2(1, 0);
        float stretchLength = min(speed * StretchFactor, MaxStretchLength);
        float length = size * aspect + stretchLength;
        float thickness = size;

        float2 dir = float2(cos(atan2(velocity.y, velocity.x)), sin(atan2(velocity.y, velocity.x)));
