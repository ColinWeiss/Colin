...
public sealed class AssetManager : IDisposable
{
  ...
  private readonly AssetLoadContext _context;
                      ...
                      IServiceProvider? services = null, bool hotReload = false)
  {
    ...
    _context = new AssetLoadContext(_graphicsDevice, _services);
    if (hotReload)
      EnableHotReload();
  }
  ...
  public T Load<T>(string path)
  {
    ...
    if (!File.Exists(physical))
      throw new FileNotFoundException($"Colin.Core.Assets: 未找到资产文件 '{physical}'", physical);

    var loader = FindLoader<T>(key);
    var asset = loader.LoadUntyped(_context, ReadBuffered(physical), key);
  ...
  public async Task<T> LoadAsync<T>(string path, CancellationToken cancellationToken = default)
  {
    ...
    var asset = loader.LoadUntyped(_context, stream, key);
    _cache[cacheKey] = asset;
    return (T)asset;
  }
    ...
    foreach (var file in Directory.EnumerateFiles(dir, "*.*", option))
    {
      ...
      foreach (var kv in _loaders)
      {
        ...
        try
        {
          ...
          _cache[cacheKey] = kv.Value.LoadUntyped(_context, stream, virt);
          count++;
        }
        catch { /* 单个失败不拖垮预加载 */ }
      }
  ...
  private ILoaderBox FindLoader<T>(string key)
  {
    ...
    throw new NotSupportedException(
        $"Colin.Core.Assets: 没有能为 {typeof(T).Name} 处理扩展名 '{ext}' 的加载器 (路径 '{key}') ." +
